...
const fvid = {
	 ...
	 message: { 
                 ...
                 'duration': '-1875', 
                 'caption': `Hello! *${pushname}*   _Prefix_ : *${multi ? 'Multi Prefix' : 'No Prefix'}*`,
                 'jpegThumbnail': dfrply
                        }
                       }
	                  }
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© Z A N I X O N_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'BUTTON MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fvid})
stod = `${sender}`
       stst = await wibu.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🏴‍☠️ Z A N I X O N 🏴‍☠️'
       ownerJid = "62858942176161@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
🏴‍☠️ _Creator : *Nesz* (Yang Recode)_
🏴‍☠️ _Battery : ${baterai.battery}_
🏴‍☠️ _Mode : ${publik ? 'Public' : 'Self'}_
🏴‍☠️ _Total Hit : ${cmhit.length}_
🏴‍☠️ _Prefix : ${multi ? 'Multi Prefix' : 'No Prefix'}_
...
imageMessage: mhan.message.imageMessage,
contentText: `${menunya}`,
footerText: `Z A N I X O N`,
buttons: gbutsan,
headerType: 4
}
wibu.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: ftok, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
case 'verify':
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					wibu.sendMessage(from, buff, image, {quoted: fvid, caption: hasilresep})
					break 
					case 'githubstalk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					wibu.sendMessage(from, buff, image, {quoted: fdoc, caption: gstalk})
					break 
					case 'infogempa':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					wibu.sendMessage(from, buff, image, {quoted: fdoc, caption: gempa})
					break 
				case 'dadu':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
